...
Copy-Item -Path "\\Vapp00066\c$\OpenSSH-Win64\*" -Destination "C:\Program Files\OpenSSH\" -Recurse -Force
...
$scriptdir = "C:\Program Files\OpenSSH\"
...
If ($PSVersiontable.PSVersion.Major -le 2) {$PSScriptRoot = $scriptdir}
Import-Module $scriptdir\OpenSSHUtils -Force
...
New-Service -Name open-ssh-agent -DisplayName "OpenSSH Authentication Agent" -BinaryPathName `"$sshagentpath`" -Description $agentDesc -StartupType Manual | Out-Null
sc.exe sdset open-ssh-agent "D:(A;;CCLCSWRPWPDTLOCRRC;;;SY)(A;;CCDCLCSWRPWPDTLOCRSDRCWDWO;;;BA)(A;;CCLCSWLOCRRC;;;IU)(A;;CCLCSWLOCRRC;;;SU)(A;;RP;;;AU)"
sc.exe privs open-ssh-agent SeAssignPrimaryTokenPrivilege/SeTcbPrivilege/SeBackupPrivilege/SeRestorePrivilege/SeImpersonatePrivilege
...
New-Service -Name open-sshd -DisplayName "OpenSSH SSH Server" -BinaryPathName `"$sshdpath`" -Description $sshdDesc -StartupType Manual | Out-Null
sc.exe privs open-sshd SeAssignPrimaryTokenPrivilege/SeTcbPrivilege/SeBackupPrivilege/SeRestorePrivilege/SeImpersonatePrivilege
...
Set-Service "open-sshd" -StartupType Automatic -Verbose
Start-Sleep 5
Start-Service "open-sshd" -Verbose
Start-Sleep 5
if(Test-Path "C:\ProgramData\ssh\sshd_config"){
    Copy-Item "C:\ProgramData\ssh\sshd_config" -Destination "C:\ProgramData\ssh\sshd_config.ORIGINAL" -Verbose
    $CustomFileContent = Get-Content "C:\Program Files\OpenSSH\sshd_config.linedata"
    $CustomFileContent | Set-Content "C:\ProgramData\ssh\sshd_config" -Force -Verbose
    New-ItemProperty -Path "HKLM:\SOFTWARE\OpenSSH" -Name DefaultShell -Value "C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe" -PropertyType String -Force
    Restart-Service open-sshd -Verbose
}
